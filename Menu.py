...
class RemplissageWindow(tk.Toplevel):
    def __init__(self, master=None):
        super().__init__(master)
        self.title("Remplissage")
        self.geometry("900x700")

        label = ttk.Label(self, text="Interface de remplissage (à remplir plus tard)")
        label.pack(pady=20)
...
class MainMenu(tk.Tk):
    def __init__(self):
        ...
        # e.g. transformations, rasterization, Bézier curves, etc.

    # -------------------------------
    # Callbacks
    # -------------------------------
    def open_decoupage(self):
        """Open the découpage interface"""
        DecoupageWindow(self)

    def open_remplissage(self):
        """Open the remplissage interface"""
